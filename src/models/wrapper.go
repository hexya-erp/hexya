...
func RegisterRecordSetWrapper(modelName string, obj any) {
	...
	if typ.Field(0).Type != reflect.TypeFor[*RecordCollection]() {
...
func (rc *RecordCollection) Wrap(modelName ...string) any {
...
// - modelName must be the name of a model that exists in the registry
func RegisterModelDataWrapper(modelName string, obj any) {
	Registry.MustGet(modelName)
	typ := reflect.TypeOf(obj)
	if typ.Kind() != reflect.Struct {
		log.Panic("trying to register a non struct type as Wrapper", "modelName", modelName, "type", typ)
	}
	if typ.Field(0).Type != reflect.TypeFor[*ModelData]() {
