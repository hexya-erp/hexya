	...
	Convey("Testing cache operation", t, func() {
		So(SimulateInNewEnvironment(security.SuperUserID, func(env Environment) {
			...
			Convey("Reading M2M fields should work both ways", func() {
				...
				So(env.cache.m2mLinks["PostTagRel"], ShouldContainKey, [2]int64{post2.ids[0], tags.ids[0]})
				So(env.cache.m2mLinks["PostTagRel"], ShouldContainKey, [2]int64{post2.ids[0], tags.ids[1]})
				...
				So(env.cache.checkIfInCache(postModel, post2.ids, []string{"Tags"}, "", false), ShouldBeTrue)
...
`)
	...
	Convey("Testing prefetch", t, func() {
		So(SimulateInNewEnvironment(security.SuperUserID, func(env Environment) {
			...
			So(env.cache.checkIfInCache(users.Model(), userSet.ids, []string{"Name"}, "", false), ShouldBeFalse)
