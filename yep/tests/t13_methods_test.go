	...
	Convey("Testing non stored related fields", t, func() {
		models.SimulateInNewEnvironment(security.SuperUserID, func(env models.Environment) {
			...
			Convey("Checking that we can search PMoney directly", func() {
				userJane := pool.User().NewSet(env).Search(pool.User().Email().Equals("jane.smith@example.com"))
				userWill := pool.User().NewSet(env).Search(pool.User().Email().Equals("will.smith@example.com"))
				pmoneyUser := pool.User().NewSet(env).Search(pool.User().PMoney().Equals(12345))
				So(pmoneyUser.Len(), ShouldEqual, 1)
				So(pmoneyUser.Ids()[0], ShouldEqual, userJane.Ids()[0])
				pUsers := pool.User().NewSet(env).Search(pool.User().PMoney().Equals(12345).Or().PMoney().Equals(5100))
	...
	Convey("Testing embedded models", t, func() {
		models.ExecuteInNewEnvironment(security.SuperUserID, func(env models.Environment) {
			Convey("Adding a last post to Jane", func() {
				...
				pool.User().NewSet(env).Search(pool.User().Email().Equals("jane.smith@example.com")).SetLastPost(postRs)
			})
	...
	Convey("Testing mixed in models", t, func() {
		models.SimulateInNewEnvironment(security.SuperUserID, func(env models.Environment) {
			Convey("Checking that mixed in functions are correctly inherited", func() {
				janeProfile := pool.User().NewSet(env).Search(pool.User().Email().Equals("jane.smith@example.com")).Profile()
