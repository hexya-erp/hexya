...
func declareMethods() {
	models.CreateMethod("Test__User", "PrefixedUser",
	...
	models.CreateMethod("Test__User", "DecorateEmail",
		`DecorateEmail is a sample method layer for testing`,
		func(rs pool.Test__UserSet, email string) string {
			return fmt.Sprintf("<%s>", email)
		})

	models.ExtendMethod("Test__User", "DecorateEmail",
	...
	models.CreateMethod("Test__User", "computeAge",
	...
	models.CreateMethod("Test__AddressMixIn", "SayHello",
		`SayHello is a sample method layer for testing`,
		func(rs pool.Test__AddressMixInSet) string {
			return "Hello !"
		})

	models.CreateMethod("Test__AddressMixIn", "PrintAddress",
		`PrintAddressMixIn is a sample method layer for testing`,
		func(rs pool.Test__AddressMixInSet) string {
			return fmt.Sprintf("%s, %s %s", rs.Street(), rs.Zip(), rs.City())
		})

	models.CreateMethod("Test__Profile", "PrintAddress",
		`PrintAddress is a sample method layer for testing`,
		func(rs pool.Test__ProfileSet) string {
			res := rs.Super()
			return fmt.Sprintf("%s, %s", res, rs.Country())
		})

	models.ExtendMethod("Test__AddressMixIn", "PrintAddress", "",
		func(rs pool.Test__AddressMixInSet) string {
			res := rs.Super()
			return fmt.Sprintf("<%s>", res)
		})

	models.ExtendMethod("Test__Profile", "PrintAddress", "",
		func(rs pool.Test__ProfileSet) string {
			res := rs.Super()
			return fmt.Sprintf("[%s]", res)
		})

	models.CreateMethod("Test__ActiveMixIn", "IsActivated",
		`IsACtivated is a sample method of Test__ActiveMixIn"`,
		func(rs pool.Test__ActiveMixInSet) bool {
			return rs.Active()
		})
}
