...
[source,go]
----
customer := pool.ResPartner().Create(env, pool.ResPartnerData{
    Name: "Jane Smith",
...
[source,go]
----
partner := pool.ResPartner().Search(env, pool.ResPartner().Where().Company().Name().Equals("NDP Systèmes"))
...
users := pool.ResUsers().Search(env, cond)
----
...
[source,go]
----
noReplyUser := pool.ResUsers().Search(env, pool.ResUsers().Email().Equals("no-reply@ndp-systemes.fr")).Limit(1)
partners := pool.ResPartner().Search(env, pool.ResPartner().Name().ILike("John"))
...
pool.ResPartner().AddMethod("UpdateBirthday",
    `PartnerUpdateBirthday updates this partner birthday.`,
...
[source,go]
----
pool.ResPartner().Methods().UpdateBirthday().Extend(
