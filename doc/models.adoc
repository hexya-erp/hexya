...
`*(f *Field) SetOnchange(value string) *Field*`::
`*(f *Field) SetConstraint(value string) *Field*`::
`*(f *Field) SetInverse(value string) *Field*`::
...
`Constraint` string::
The name of a method to call to validate the value of this field in a record.
The value must be the name of a method on this RecordSet with the following
signature:
...
`Inverse` string::
Declares a inverse method for a computed field. This method will be called when
the field is set. The given method must have the following signature:
