...
`*Commit()*`::
`*Rollback()*`::
Commits (respectively rolls back) the current transaction.

WARNING: Do NOT call `Commit()` or `Rollback()` on Environment instances that
you did not create yourself with `NewEnvironment()`. The framework will
...
`*NewEnvironment()*`::
NewEnvironment returns a new Environment with the given parameters in a new DB
transaction.
...
NOTE: Except for launching asynchronous jobs, there is usually no need to call
`NewEnvironment()`. Check <<Modifying the Environment>> instead.
