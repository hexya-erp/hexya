...
[source,go]
----
customer := pool.ResPartner().NewSet(env).Create(pool.ResPartner{
    Name: "Jane Smith",
...
[source,go]
----
partner := pool.ResPartner().NewSet(env).Filter("Company.Name", "=", "NDP Systèmes")
...
[source,go]
----
noReplyUser := pool.ResUsers().NewSet(env).Filter("Email", "=", "no-reply@ndp-systemes.fr").Limit(1)
partners := pool.ResPartner().NewSet(env).Filter("Name", "ilike", "John")
...
[source,go]
----
resPartnerModel := models.Registry.MustGet("ResPartner")
...
All methods definitions MUST be made in the `init()` the main package or of
...
`*models.NewModel(modelName string, dataStructPtr interface{}) *Model*`::

Creates a new model with the given `modelName` and add the fields of
dataStructPtr to the model. It returns an pointer to the created model
instance.

[source,go]
----
course := models.NewModel("Course", new(struct {
...
`*models.NewMixInModel(modelName string, dataStructPtr interface{}) *Model*`::
...
Creates a new transient model with the given name and adds all fields of the given
`dataStructPtr` to the model. Transient model instances have a limited life
time and are automatically removed from database. They are mainly used for
wizards.
...
==== Model Extension
`*(*Model) Extend(dataStructPtr ...interface{})*`::
Extend this model by adding fields of `dataStructPtr` to the model.
...
[IMPORTANT]
====
The `methodName` attributes of `CreateMethod` *MUST* be string
literals for `yep-generate` to work correctly. Do *NOT* use variable or
constant here.
====
