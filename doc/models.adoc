...
course.AddFields(map[string]models.FieldDeclaration{
...
session.AddFields(map[string]models.FieldDeclaration{
...
where `valueType` is the go type for the given field values.
